...
来考虑一个房价预测的简单例子。如果你要预测某套房子的价格，需要考虑哪些因素？可能包括房子的面积、卧室数量、卫生间数量、房龄、距地铁站距离，等等。这里每个因素就被称为一个**特征**（Feature）。将所有特征组合起来，就得到了一个特征向量（Feature Vector，与下矩阵中的[特征向量](../linear/matrices.md#特征向量与特征值)不是同一个概念）：
