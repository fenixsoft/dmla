...
计算表明，如果沿梯度方向 $(2, 1)$ 走，每单位距离函数值增加约 2.24（这是所有方向中最大的）。如果沿负梯度方向 $(-2, -1)$ 走，每单位距离函数值减少约 2.24（这是所有方向中"下降最快"的）。如果沿与梯度垂直的方向走（比如 $(1, -2)$），函数值不变，因为这正是等高线的切线方向。相应地，负梯度方向就是函数值下降最快的方向。这就引出机器学习中梯度下降的核心思想：沿着负梯度方向移动，可以最快地找到函数的最小值。
