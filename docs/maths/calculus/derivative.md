...
第三个条件将极限值与函数值统一起来，"极限等于函数值"正是连续的真正含义。连续函数有许多良好的性质。譬如，[介值定理](https://en.wikipedia.org/wiki/Intermediate_value_theorem)（Intermediate Value Theorem）告诉我们如果连续函数 $f$ 在区间 $[a, b]$ 上取值 $f(a)$ 和 $f(b)$，那么对介于 $f(a)$ 和 $f(b)$ 之间的任何值 $c$，存在 $x \in (a, b)$ 使得 $f(x) = c$。这个定理常用于数值计算中求方程的根（如二分法）。
...
在机器学习中，我们通常希望最小化的损失函数是凸函数（Convex Function），二阶导数非负，函数图像"碗口向上"，这意味着有唯一的全局最小值。判断函数的凹凸性，对于后面判断优化算法（如梯度下降）的能否稳定收敛十分重要。继续推广，对于某些函数，我们还可以计算三阶、四阶甚至更高阶的导数。譬如：
