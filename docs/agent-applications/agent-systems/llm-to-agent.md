...
语言模型擅长"说"，智能体（Agent）擅长"做"。让一个只会生成文本的系统学会自主行动，这个跨越并不是最近才开始的尝试。早在人工智能研究的初期，研究者们就梦想着建造能感知环境、制定计划、执行行动的自主系统。1950 年代，美国空军上校约翰·博伊德（John Boyd）提出了 OODA 循环（Observe-Orient-Decide-Act）的概念，用于描述战斗机飞行员在空战中的决策过程：观察敌机位置、理解战场态势、决定机动策略、执行操作，然后根据新状态重新开始循环。博伊德的核心洞察是：在对抗中，谁能更快地完成这个循环，谁就占据主动权。这个框架后来被广泛借用到 AI 领域，成为智能体架构设计的核心参考。
...
从 LLM 到 Agent 的跨越，本质上是把"单轮问答"升级为"多轮循环决策"。一个只会生成文本的模型，如何变成一个能自主完成复杂任务的系统？这篇文章将从 LLM 的能力边界出发，逐步展开 Agent 的核心架构、自主行动机制和设计原则，最终通过代码实践构建一个最小但完整的 ReAct Agent。

## LLM 的能力边界

在讨论 Agent 之前，我们需要先诚实地审视语言模型本身"不能做什么"。理解这些能力边界，才能理解 Agent 架构中每一个组件存在的理由。

### 生成式模型的被动性

想象这样一个场景：你打开终端，输入"帮我检查一下服务器状态"。如果对面是一个大语言模型，它会怎么做？它会生成一段文字回复，可能是"你可以使用 `top` 命令查看 CPU 使用率"这样的建议，也可能直接生成一段模拟的服务器状态文本。但它不会真的登录服务器、执行命令、读取输出，因为它根本没有这个能力。

这就是大语言模型最根本的局限：它是一个被动的文本生成器。给它一段输入，它输出一段文本，任务就结束了。模型的内部运作可以描述为"输入 token 序列 → 输出 token 序列"的概率采样过程，输出的是语言符号，而非操作指令。说"帮我发一封邮件"和真正发出一封邮件之间，横亘着一条行动鸿沟（Action Gap）：模型能生成邮件正文，但无法连接 SMTP 服务器、填写收件人地址、点击发送按钮。

这条鸿沟并非设计缺陷。语言模型的设计目标就是专注于语言的理解与生成，行动能力通过外部机制来补全。这就好比一个聪明但瘫痪的大脑：它可以思考和分析，但要真正移动手臂，还需要神经系统和肌肉的配合。Agent 架构中的"工具调用"（Tool Use）机制扮演的正是神经系统的角色，将模型的意图转化为外部世界的实际操作。

被动的另一个后果是知识的静态性。模型的知识截止于训练数据的末尾，对于训练后发生的事件、私有数据库中的记录、实时变化的传感器读数，模型无从得知。这意味着即便模型有能力生成正确的行动指令，它也可能因为信息过时或不完整而做出错误决策。这是 Agent 架构需要外部记忆和信息检索能力的根本原因。

### 上下文窗口的局限

即使解决了行动能力的问题，大语言模型还有另一个硬性约束：上下文窗口。上下文窗口决定了模型在一次推理中能"看到"多少信息，而这个容量是有限的。

以编程任务为例，一个持续数小时的编码会话会迅速积累大量信息：用户的需求描述、多轮对话的调整指令、多个源文件的内容、终端的命令输出、调试日志、错误堆栈……这些信息中的每一项都可能对后续决策至关重要，但窗口的物理容量不允许全部保留。更棘手的是，上下文窗口的"遗忘"是硬性的：超出窗口上限的信息对模型而言完全不存在，而非像人类记忆那样模糊但仍然可及。这种硬遗忘不同于人类的"记不太清了"：对于模型来说，被挤出窗口的信息等于从未出现过。

上下文窗口的利用效率本身也随长度增加而下降。研究者发现了一个被称为"迷失在中间"（Lost in the Middle）的现象：模型对长上下文开头和结尾的信息理解较好，但对中间部分的信息检索准确率显著下降。这意味着简单地扩大上下文窗口并不能从根本上解决问题，反而可能引入新的可靠性风险。

这三个特性共同指向一个结论：Agent 系统需要外部记忆。外部记忆可以采用向量数据库存储历史交互的嵌入向量，也可以使用结构化摘要压缩关键信息，或者将长期记忆与短期工作记忆分层管理。

### 幻觉与不可靠性

语言模型偶尔会"一本正经地胡说八道"，这个现象被称为幻觉（Hallucination）。它的根源在于语言模型优化的目标是文本的流畅性和连贯性，而非事实的准确性。模型在生成每个 token 时，选择的是在统计意义上最"合理"的下一个词，而不是最"正确"的那个词。

在聊天场景中，幻觉通常只是让人啼笑皆非：编造一个不存在的餐馆、虚构一本从未出版的书。但在 Agent 场景中，幻觉的后果要严重得多。假设一个 Agent 负责操作云服务器，它可能幻觉出一条不存在的 API 调用，将这条调用发送给云平台，然后根据返回的错误信息进一步做出错误的"修正"决策。一个看似微小的幻觉可以像滚雪球一样在 Agent 的决策循环中被放大，导致整个任务的偏离。

更根本的问题在于，幻觉无法被彻底消除。语言模型的概率本质决定了它总有一定概率生成不符合事实的内容，任何声称"零幻觉"的方案都需要审慎对待。务实的态度是接受幻觉的可能性，然后通过 Agent 的多层验证机制来检测和修正它：工具调用的返回值可以校验模型生成的操作是否合理，自我反思（Self-Reflection）步骤可以检查推理链路中的逻辑漏洞，外部知识源的交叉验证可以提供独立的事实确认。

理解幻觉的存在，是设计 Agent 容错机制的前提。一个设计良好的 Agent 不是"永不犯错"的 Agent，而是在犯错后能发现错误并自我修正的 Agent。

## Agent 的核心架构

理解了 LLM 的能力边界之后，我们来看 Agent 架构如何逐一弥补这些不足。Agent 的核心架构设计围绕一个根本问题展开：如何将一个被动的文本生成器转变为一个能自主决策的行动系统？

### 感知 - 决策 - 行动循环

如果用一个词概括 Agent 的运行方式，那就是"循环"。Agent 不是一个一次性的问答机器，而是一个不断重复"观察 → 思考 → 行动"的决策系统。

这个循环的思想来源可以追溯到前面提到的约翰·博伊德（John Boyd）。他在 1950 年代研究空战决策时提出的 OODA 循环，最初用于描述飞行员如何在瞬息万变的战场环境中做出快速决策。Agent 继承了 OODA 循环的基本框架，但赋予了每个环节新的含义。LLM 在循环中扮演的是"思考与决策"的角色：它接收环境状态的结构化描述，推理当前处境，生成下一步行动的方案。然后由执行引擎（Executor）将行动方案转化为实际操作，调用 API、读写文件、查询数据库，并收集操作结果作为新的观察，反馈给 LLM 进行下一轮推理。
...
这个循环看似简单，但其中隐含着一个关键挑战：停止条件。Agent 如何知道任务已经完成？如果目标描述不够精确（"帮我优化代码"），Agent 可能陷入无休止的优化循环；如果过早认为任务完成，又可能遗漏重要步骤。停止条件的判断本身就需要推理能力，这正是 Agent 与简单脚本的本质区别：脚本按固定步骤执行到结束，Agent 需要在执行过程中动态判断目标是否达成。

从更广阔的视角看，OODA 循环与强化学习中的 Agent-Environment 交互模式在结构上是一致的。区别在于表示方式：强化学习使用数值向量（状态向量、动作向量、奖励值），而 LLM Agent 使用自然语言文本作为状态描述和动作描述。语言的表达能力远强于数值向量，这意味着 LLM Agent 可以处理远比传统强化学习智能体更复杂、更开放的任务，但代价是语言描述的歧义性也为决策引入了新的不确定性。

### ReAct 模式

OODA 循环给出了 Agent 运作的宏观框架，但具体到"LLM 如何在每一步做出决策"，还需要更精细的模式设计。

2022 年，姚顺雨（Shunyu Yao）等人在论文《ReAct: Synergizing Reasoning and Acting in Language Models》中提出了 ReAct 模式，首次将推理（Reasoning）和行动（Acting）在语言模型内部统一为一个交替进行的流程。ReAct 的核心思想非常朴素：让模型在行动之前先"想一想"，在行动之后根据结果再"想一想"，如此反复。

具体来说，ReAct 的每一步包含三个环节：思考（Thought）阶段，模型用自然语言分析当前状态并决定下一步做什么；行动（Action）阶段，模型生成一个工具调用指令，由外部系统执行；观察（Observation）阶段，工具的执行结果以文本形式追加到上下文中，成为下一轮思考的输入。这个 Thought → Action → Observation → Thought 的循环周而复始，直到模型判断任务已经完成。
...
为了理解 ReAct 为什么有效，可以对比另外两种极端策略。纯推理模式（Chain-of-Thought，CoT）让模型一步一步推理，但不给它调用工具的能力。这好比让一个人蒙着眼睛解题：他可以一步步思考，但无法查阅任何外部资料来验证自己的想法。纯行动模式（Act-only）则反过来，让模型直接调用工具但不在中间步骤进行显式推理，导致模型在信息不充分时盲目行动。
