...
| deep-learning/convolutional-neural-network/alexnet-experiment.md | 翻译完成 | ✓ | | LaTeX 不变；内部 `.md` 链接保留原相对路径不变；锚点 `#数据管理`→`#data-management`、`#网络结构`→`#network-architecture`；代码注释和 print 译英；图片路径 `./assets/`→`../../../deep-learning/convolutional-neural-network/assets/` |
