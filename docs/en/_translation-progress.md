...
> 最后更新: 2026-07-25（alignment-new-paradigms.md 校审完成）
...
| language-models/alignment/llm-dpo-experiment.md | 翻译完成 | | | 锚点译英（`#数据管理`→`#data-management`）；内部 `.md` 链接保留原相对路径；LaTeX 不变；代码注释译英；`::: info` `::: details` 标题译英；JSON 示例内容译英；表格内容译英 |
