...
| language-models/pretraining/distributed-training.md | 翻译完成 | | | 锚点译英（`#第三阶段-预训练`→`#phase-3-pretraining`）；内部 `.md` 链接保留原相对路径；图片路径 `./assets/`→`../../../language-models/pretraining/assets/`；Mermaid 标签译英；代码无；LaTeX 不变 |
