...
- **Cloud-Native Deployment:** Deploying LLM inference services on Kubernetes leverages the cloud platform's elastic scaling capabilities to handle traffic fluctuations. However, cloud-native deployment of GPU workloads faces a series of engineering challenges, which is why current AI cloud services still struggle to manage GPU workloads using native Kubernetes directly — often requiring deep customization or in-house solutions (for example, as mentioned in the discussion of PD disaggregation architecture, Kimi uses its own Mooncake system to manage the KV Cache).

    Node pool management is the first practical challenge. A Kubernetes cluster typically hosts both CPU and GPU node pools, and inference Pods must be precisely placed on GPU nodes through node selectors or taints and tolerations. GPU nodes are far more expensive than CPU nodes — the cheapest cloud server costs only a few dozen dollars per year, while a single A100 costs tens of thousands of dollars — so node pool sizing requires careful planning; any oversight leads to significant idle costs. Model weight persistence is another unavoidable pain point. When an inference container restarts, it needs to reload tens of GB of weights. Embedding them directly in the container image would bloat the image to tens or even hundreds of GB, making pull times unacceptably long. A better approach is to store the weights on a persistent volume or in object storage (such as S3), with the container loading them from external storage on startup. The initial load still takes tens of seconds to minutes, but weight sharing mechanisms can significantly accelerate subsequent loads.

    Elastic scaling poses the greatest difficulty for GPU deployment. Kubernetes' HPA can automatically scale replicas based on GPU utilization, but a GPU Pod must go through a chain of stages from creation to readiness: node scheduling (seconds), image pulling (tens of seconds to minutes), weight loading (tens of seconds to minutes), and GPU warmup (seconds) — the entire process can take 3 to 5 minutes. Yet online traffic bursts often arrive within just a few seconds. By the time the new Pod is finally ready, the traffic peak has already passed, or the existing nodes have already been overwhelmed. The gap between cold-start latency and traffic surge speed is fundamentally mismatched. The challenge in the scale-down direction is equally critical. When traffic drops and instances need to be reduced, in-flight requests cannot simply be terminated, or users will receive truncated replies. Kubernetes' graceful shutdown mechanism allows a Pod to finish handling current requests before exiting after receiving a termination signal, but this requires the scheduling layer to route new requests to other instances in a timely manner, forming a complete drain-and-switch workflow.
