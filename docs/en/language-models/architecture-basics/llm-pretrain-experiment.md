...
The most obvious limitation is that the pretrained model can only do text continuation, not conversation. Its training objective is to predict the next word given preceding context. Given the input "Hello," the model naturally treats these words as the beginning of a passage and continues with "Hello, welcome to..." rather than recognizing this as a conversation opener and responding with a greeting. This understanding of conversational format is not something pretraining can impart -- it requires the SFT phase, with extensive demonstrations in the `<|im_start|>user\n...<|im_end|>\n<|im_start|>assistant\n` format, to specifically teach the model how to engage in dialogue.

Another fundamental limitation lies in how knowledge is stored. The pretrained model's knowledge is dissolved within its parameter weights, unlike a database that allows precise queries. The model might correctly continue a fact under the right prompt, but it cannot tell you the source of that fact, nor can it guarantee it won't fabricate a different version under a slightly different prompt -- this is precisely the root cause of LLM hallucination. Beyond that, the pretraining phase only cultivates the most basic language modeling abilities. Advanced skills such as tool calling, chain-of-thought reasoning, and instruction following all need to be grafted on during subsequent alignment training stages like SFT and RLHF.
