...
- First, pretraining determines the capability ceiling. The model's pretraining capability sets the upper bound for inference scaling. A 1B model, no matter how much computation is invested, would struggle to match the performance of a 100B model on complex reasoning tasks, because its "knowledge reserve" and "reasoning potential" are inherently limited. This is like someone who has only learned basic arithmetic -- no matter how much time they are given to think, they would struggle to solve calculus problems.

- Second, inference scaling approaches the ceiling. Within the range of pretraining capability, inference scaling helps the model better realize its existing potential. A powerful model that only answers directly is like a student who writes only the final answer without showing the work on an exam -- even if they have mastered the knowledge, they might answer incorrectly due to carelessness or insufficient reasoning budget. Inference scaling is like giving the model a scratch paper, allowing it to demonstrate its full capability under favorable conditions.

- Third, cost trade-offs determine strategy choice. Pretraining is a one-time fixed cost -- train once, benefit long-term. Inference is a variable marginal cost -- you pay each time you use it. For high-frequency, low-value tasks (such as customer service conversations), investing more in pretraining and less in inference is more economical. For low-frequency, high-value tasks (such as mathematical proofs, code audits), investing more in inference may be more cost-effective.
...
- **Fixed budget allocation**: Divide problems into several difficulty levels, with each level assigned a fixed amount of computation. Simple problems get 1 sample, medium problems get 4, difficult problems get 16. This strategy is easy to implement but not precise enough. Different problems within the same difficulty level may require vastly different amounts of computation, and fixed allocation either wastes resources or falls short.

- **Compute-optimal scaling**: For problems of different difficulty levels, the optimal reasoning strategy is entirely different. For simple problems (where the model has a high pass rate), adopt a sequential revision strategy -- the model's initial answer is already close to correct, so the model just needs to review its response and correct minor errors. For difficult problems (where the model has a low pass rate), adopt a parallel search ([tree search](#tree-search) / [Best-of-N](#best-of-n-sampling)) strategy -- the model's initial answer is likely wrong, so it needs to explore from different directions, generate multiple independent candidates, and select the best one.

- **Progressive allocation**: Start with a small amount of computation (e.g., generate an initial answer), evaluate the quality of the current answer based on the preliminary result, and gradually increase computation if it is not good enough. This is similar to an exam strategy -- quickly skim the questions to gauge difficulty, then decide how much time to invest. Progressive allocation complements the dynamic stopping strategy for adaptive computation discussed later. Progressive allocation determines when to increase computation, while dynamic stopping determines when to stop reasoning.
...
- **Confidence threshold**: The model evaluates its confidence in the current answer after each reasoning step. If the confidence is sufficiently high, output the answer; otherwise, continue reasoning. Setting the confidence threshold is a trade-off decision: a higher threshold typically yields higher accuracy but longer inference time; a lower threshold speeds up inference but may sacrifice accuracy. In practice, the threshold is usually adjusted dynamically based on task type and performance requirements.

- **Compute budget prediction**: The model predicts the amount of computation required to complete the task before starting to reason, then allocates the corresponding compute budget. This is similar to a project manager estimating workload before a project starts -- simple tasks get fewer resources, complex tasks get more. The advantage of compute budget prediction is that planning can be done before reasoning begins, avoiding repeated judgments during the reasoning process about whether to continue. However, the difficulty lies in accurately predicting problem difficulty, which is precisely the problem that [complexity assessment](#complexity-assessment) aims to solve. Assessments based on model pass rates or internal signals can provide input for budget prediction.

- **Dynamic stopping**: The model dynamically decides whether to stop during the reasoning process. Unlike confidence thresholds, dynamic stopping considers not only the quality of the current answer but also whether continuing to reason is likely to bring improvement. This requires a stopping judge that evaluates whether the current answer is good enough and whether the expected benefit of continued reasoning justifies the additional computational cost.
...
- **One-time fixed cost vs. variable marginal cost**: Pretraining is a fixed cost; inference is a variable cost. For a model serving millions of users, investing more in pretraining may be more economical. For a model used only in high-value scenarios, investing more in inference is more cost-effective.
- **General capability vs. specialized capability**: Pretraining improves general capability; inference improves performance on specific tasks. If the model needs to perform well across a wide range of tasks, pretraining investment takes priority. If extreme performance is only needed on specific tasks, inference investment is more effective.
- **Intersection of marginal returns**: As mentioned earlier, Snell et al.'s experiments showed that increasing inference computation by 4 times can, on certain tasks, compensate for a 14x gap in parameter count. This equivalence provides a quantitative reference: when the inference budget is sufficient and the task is specific, "small model + strong inference" may be more economical than "large model + weak inference."
