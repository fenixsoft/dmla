...
The following code implements SFTDataset, which converts dialogue data into a model-trainable format. The main logic is to locate the AI assistant's response intervals and generate corresponding masks. This code is called during training and does not need to be run manually.
