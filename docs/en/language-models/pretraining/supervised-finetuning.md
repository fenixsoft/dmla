...
After SFT training, the model understands the conversational intent and role division. The user is responsible for asking questions, and the model is responsible for answering. The model knows that when faced with a question, it should provide direct, useful information rather than continuing the text. The foundation model learns the probability distribution of text, while the instruct model learns the behavioral pattern of following instructions. The most fundamental role of SFT is to establish the behavioral framework for conversation. It enables the model to complete the identity shift from "continuation" to "answering" -- this is the foundation for all subsequent work. Without this shift, RLHF would be impossible, because the reward model evaluates the quality of "answers," not "continuations"; the model must first learn to "answer" before it can be "graded." Moreover, SFT provides a stable and convergent starting point for the RLHF stage. If one were to jump directly from the base model to RLHF, the output distribution gap between the reward model and the policy model would be too large, making the training process difficult to stabilize. SFT first brings the model to a level where it can answer questions, and RLHF then further polishes it from there, making the entire pipeline smooth and controllable. On top of this foundation, through carefully designing the distribution of instruction data, SFT can also selectively inject domain-specific knowledge and skills into the model. For example, if the training data contains more programming Q&A pairs, the model's coding ability will correspondingly improve -- the data mix itself serves as an implicit curriculum.
...
By updating only 0.39% of the parameters, LoRA can achieve results close to full-parameter fine-tuning. This remarkable efficiency is precisely why LoRA quickly became an industry standard. In its implementation, LoRA involves two clever design choices. One is the initialization strategy: matrix $A$ is initialized with a random Gaussian distribution, while matrix $B$ is initialized as a zero matrix. This means that at the start of training, $BA = 0$, the output of the LoRA bypass is completely zero, and the model's behavior is identical to the original pre-trained model in every respect. Fine-tuning does not begin from some random perturbation, but rather from the capabilities already acquired by the pre-trained model, making the training process more stable and controllable. The other is the scaling factor: LoRA multiplies the bypass output by $\alpha / r$ to control the magnitude of the update, where $\alpha$ is a hyperparameter typically set to 1 to 2 times $r$. When $\alpha = r$, the scaling factor is 1, and the bypass adds no amplification or suppression. When $\alpha = 2r$, the bypass output magnitude is doubled, which is equivalent to implicitly raising the learning rate during fine-tuning. This design allows researchers to independently adjust the influence of the LoRA bypass on the entire network without modifying the base learning rate.
