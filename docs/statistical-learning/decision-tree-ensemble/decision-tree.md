...
上述三种分裂准则分别对应三种经典的决策树算法：ID3 采用信息增益，C4.5 采用增益率，而 CART 则使用 Gini 指数。下面逐一介绍这些算法的核心思想和实现细节。
