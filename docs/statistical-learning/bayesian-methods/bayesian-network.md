...
三类推断的关系可以概括为：概率查询回答"概率分布是什么"，MAP 回答"查询变量集的最佳取值组合是什么"，MPE 回答"整个网络的最佳状态配置是什么"。贝叶斯网络最常用的推断方法是**枚举法**（Enumeration Inference）。其核心思想是：利用贝叶斯网络的分解特性，通过枚举所有与证据一致的隐藏变量赋值，计算查询变量的后验分布。
具体步骤如下：
...
- **逆向推断**：已知肺癌后，吸烟概率从 30% 提升到 81.08%，体现了贝叶斯的"逆向推理"能力。
