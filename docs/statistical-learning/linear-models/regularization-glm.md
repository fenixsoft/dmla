...
计算机科学家亚瑟·塞缪尔（Arthur Samuel）在开发机器学习跳棋程序时，发现程序在训练集上表现得很好，但在实战中却频频失误，原因是模型过度学习训练数据的细节，丧失了对真实规律的把握能力。就好比中学生学习选择了背诵历届高考原题，而不是真正理解知识，自然在考试中难以获得好成绩。塞谬尔将这种现象命名为**过拟合**（Overfitting），**正则化**（Regularization）则是用来解决过拟合问题的关键技术，它通过在损失函数中加入参数约束，强迫模型跳出训练数据的复杂细节，寻找更简单的结构来拟合数据。
...
2. **参数收缩稳定**：正则化项 $\lambda\beta$ 使得参数估计值比 OLS 小，参数被拉向原点。这种收缩效应降低了模型对单个特征的依赖，提升了泛化稳定性。
