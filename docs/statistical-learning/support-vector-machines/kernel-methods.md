...
- **线性核（左图）**：假设高风险与低风险客户之间存在线性边界。这在现实中常常不成立，因为客户的还款能力受多重因素交互影响。线性核的准确率通常较低，无法捕捉复杂的财务行为模式。
- **多项式核（中图）**：通过二次映射捕捉特征间的交互关系（如"高负债比例 + 年轻年龄"的组合风险）。这更符合现实逻辑：金融风险常常不是单一因素决定的，而是多个因素组合的结果。多项式核相比线性核有明显提升。
- **RBF 核（右图）**：展现了核技巧的真正威力。决策边界呈现为复杂的非线性曲面，能够灵活适应数据的局部分布。RBF 核通过无穷维特征空间的映射，捕捉了信用风险中难以显式建模的复杂模式，一般能获得最高的分类准确率。
