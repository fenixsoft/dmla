...
这是一本讨论"如何将机器学习应用于生产系统"的文档，帮助软件开发者在人工智能时代下梳理知识框架的导航地图。从目的角度看，它与我的另一部文档《[凤凰架构](https://icyfenix.cn/)》是一样的。不过，这部文档的阅读门槛相对凤凰架构要更高，建议开始前先阅读文章《[关于设计机器学习系统](./introduction/about-dmla.md)》，了解文档的主旨与读者画像。

笔者出于以下目的，撰写这部文档：

- 笔者是一名软件工程师，也是机器学习方向的 PhD，身处软件业正由"硬编码的逻辑"转向"数据驱动的智能"的时点，希望借文档为引，系统性地整理机器学习相关知识，查缺补漏，将它们都融入自己的知识框架之中。
- 笔者认为技术人员成长有一捷径，不仅要去看、去读、去想、去用，更要去说、去写。将自己认为掌握了的知识叙述出来，能够说得清晰有条理，讲得理直气壮；能够让他人听得明白，释去心中疑惑；能够把自己的观点交予别人的审视，乃至质疑，在此过程之中，会挖掘出很多潜藏在"已知"背后的"未知"。未有知而不行者，知而不行，只是未知。
...
- **代码沙箱**：本文档内包含大量的代码实践，用于演示机器学习算法以及进行模型训练。对于第一部分数学基础和第二部分经典统计学习算法，只需纯 CPU 环境即可运行。对于深度学习及之后的内容，有部分模型训练实验需要有 GPU 支持（目前是 PyTorch with CUDA 12.8）。更多详情请参见[构建沙箱环境](sandbox.md)。
  ```shell
