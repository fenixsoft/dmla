...
- **代码沙箱**：本文档内包含大量的代码实践，用于演示机器学习算法以及进行模型训练。对于第一部分数学基础和第二部分经典统计学习算法，只需纯 CPU 环境即可运行。对于深度学习及之后的内容，有部分实验需要有 GPU 支持（目前是 PyTorch with CUDA 12.8）。可通过如下命令一键完成沙箱环境构建。租赁 GPU 云主机可在 10 元成本内完成所有训练实践，更多详情请参见[构建沙箱环境](sandbox.md)。
  ```shell
