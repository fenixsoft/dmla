...
- **群体稳定性指数**（Population Stability Index，PSI）是工业界最广泛使用的漂移量化指标，它衡量两个分布之间的差异程度。给定参考分布 $P$（通常是训练数据上计算的基线）和当前分布 $Q$（生产数据上实时计算的分布），PSI 的计算方式如下：
    ...
    PSI 的常用阈值经验是 PSI < 0.1 表示分布基本稳定，0.1 ≤ PSI < 0.25 表示中度漂移，PSI ≥ 0.25 表示显著漂移。这些阈值是经验性的，不同业务场景和特征类型应该根据历史数据校准自己的阈值。下图是一个 PSI 计算过程的可视化的例子。左图为参考分布和当前分布在各箱子中的占比对比，右图为每个箱子对最终 PSI 值的贡献，红色虚线标注了 0.1（轻微漂移）的阈值。
