...
[微积分基础](../../maths/calculus/derivative.md)介绍了从极限到微分的理论基础，理解了导数、梯度、链式法则等核心概念。本章将这些理论转化为可执行的代码，通过 NumPy 实现数值微分、梯度计算，并介绍 PyTorch 的自动微分机制。动手实践既可以加深对概念的理解，也有助于培养解决实际问题的能力。
...
前向差分虽然简单，但精度有限。更精确的方法是**中心差分**（Central Difference），其公式为：$f'(x) \approx \frac{f(x + h) - f(x - h)}{2h}$，与前向差分只用 $x$ 和 $x + h$ 两点不同，中心差分以 $x$ 为中心，取左右对称的两点 $x + h$ 和 $x - h$，用它们的函数值差除以 $2h$。这种对称取点的方式使得截断误差更低：中心差分的误差项是 $O(h^2)$，前向差分的误差项是 $\frac{h}{2}f''(x)$，即 $O(h)$（推导略）。因此，中心差分要比前向差分的精度高一阶——当 $h$ 缩小 10 倍时，前向差分误差缩小 10 倍，而中心差分误差缩小 100 倍。以下是代码实现：
...
下面是梯度计算的代码实现，从代码中可以看到，计算一个 $n$ 维梯度需要调用函数 $2n$ 次（每个偏导数需要两次函数调用），计算开销比一元函数大许多。事实上，从便捷性和性能两方面来看，在实际机器学习场景中，人们一般使用[自动微分](#自动微分)而非数值微分来计算梯度。但理解数值微分的原理仍是必要的。
...
使用数值微分计算导数或手动实现链式法则，计算过程都很繁琐，稍有不慎就会出错；数值微分中每个变量需要调用两次函数，计算效率也不高。机器学习框架 PyTorch 提供了**自动微分**（Automatic Differentiation，简称 autodiff）功能，可以实现自动、精确、高效地计算梯度。
...
本章将微积分的理论转化为可执行的代码，建立了从数学概念到程序实现的桥梁。
 ...
 这些技术构成了机器学习的计算基础，理解这些底层机制，有助于调试和优化模型，也能帮助把握深度学习的运作原理。
