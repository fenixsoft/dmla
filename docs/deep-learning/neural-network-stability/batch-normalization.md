...
既然各层输入分布的变化是训练困难的根源，就要想办法在每一层引入一种机制，强制将输入分布标准化，使其保持稳定。这个想法实际上并不新颖。在传统机器学习中，数据标准化本来是最基本的预处理步骤。对于特征向量 $x$，我们通常将其标准化为：

$$\hat{x} = \frac{x - \mu}{\sigma}$$

其中 $\mu$ 是训练数据的均值，$\sigma$ 是标准差。标准化后的特征 $\hat{x}$ 具有零均值和单位方差的特点，这种分布更利于优化算法工作，梯度方向更稳定，学习率可以设置得更大。现在问题在于，在深度网络中，每一层的训练数据（即前一层的输出）的分布是动态变化的。我们无法预先计算出固定的 $\mu$ 和 $\sigma$，因为这些统计量随训练过程不断演变。此外，纯粹的数据标准化可能带来新的问题，如果强制将所有激活值标准化为零均值、单位方差，势必会限制网络的表示能力。Sigmoid 函数的非线性特性在输入远离零时更强，强制将输入拉向零附近，可能使网络变得过于线性。BN 的设计思路正是围绕这解决两个问题展开的。

首先，BN 用 [Batch](../neural-network-structure/forward-propagation.md#批量计算与效率优化) 统计替代全局统计。既然无法预先计算固定的 $\mu$ 和 $\sigma$，BN 在每次参数更新时，用当前 Mini-Batch 的数据来估计统计量。每个 Mini-Batch 虽然只是全部数据的一个子集，但其均值和方差足以反映当前分布的大致特征。用这些估计值进行标准化，就能在训练过程中实现每层输入的实时稳定。这意味着无论前层参数如何变化，本层接收到的数据都会被强制拉到零均值、单位方差，从而切断了参数更新 → 分布变化 → 训练不稳定这一连锁反应。

然后，引入可学习的缩放和偏移参数。纯粹的标准化会将所有激活值限制在固定的分布中，削弱网络的非线性表达能力。BN 通过在标准化后增加两个可学习参数 $\gamma$（缩放）和 $\beta$（偏移），将标准化升级为可学习的分布变换。网络可以通过学习合适的 $\gamma$ 和 $\beta$，自主决定是否需要标准化、以及标准化到什么程度。如果某个层的最佳分布就是零均值、单位方差，$\gamma$ 和 $\beta$ 会学习为单位变换。如果某个层需要特定的分布来激活非线性特性，$\gamma$ 和 $\beta$ 会将标准化后的数据调整到合适的位置和尺度。这种设计保证了 BN 不会降低网络的表达能力，它只是提供了一个更易于优化的起点，网络有权选择是否偏离这个起点。

将这两个思路结合起来，BN 在网络的每一层中嵌入了一个分布稳定器，训练时实时计算 Batch 统计量，对每层输入进行标准化，再用可学习参数微调分布。这使得各层不再需要不断适应前层带来的分布变化，可以专注于学习稳定的特征表示。BN 对每个特征维度独立操作，设 Mini-Batch 中某个特征的值为 $\{x_1, x_2, ..., x_m\}$，其中 $m$ 是 Batch Size，BN 的计算分为计算统计量、标准化、缩放偏移三个步骤。每一步都有其特定的目的和设计考量。
...
本节我们深入探讨 BN 具体如何改善深度网络的训练过程。BN 对训练的影响是多方面的，它直接解决了协变量偏移问题，间接带来了正则化效果，并改变了网络架构的结构设计选择。

BN 对训练稳定性的提升，最根本的贡献是分布稳定。每层的输入被强制标准化为零均值、单位方差，无论前层参数如何更新，本层接收到的数据始终落在相似的数值范围内，就像给每一层都装了一个稳压器，不管输入信号如何波动，经过 BN 处理后都变得平稳，协变量偏移的根本问题由此被直接消除。分布稳定之后，梯度的稳定性也随之而来。激活函数的梯度与输入值密切相关。Sigmoid 在输入远离零时落入饱和区，梯度几乎为零。ReLU 在输入为负时直接被截断，梯度彻底消失。标准化将激活值拉回到零附近，恰好落在这些激活函数的舒适工作区间，梯度适中且稳定。即使对于 ReLU，标准化也减少了持续负值的风险，降低了神经元死亡的概率。梯度稳定意味着每一步的参数更新方向都更加可靠，收敛过程自然也就更顺畅。

分布稳定还解锁了一个更直接的加速手段：使用更大的学习率。在没有 BN 的网络中，大学习率会导致各层输入分布剧烈变化，训练随时可能崩溃，这正是协变量偏移最直观的危害。但 BN 在每一层都重新标准化，即使参数大幅度更新，下一层接收到的数据仍然被约束在稳定范围内，学习率因此可以大胆提高。在 ResNet 等深度网络的训练中，学习率通常从 0.1 起步，这在没有 BN 的时代几乎是不可想象的。另一个被 BN 悄悄消解的是初始化敏感度。初始化不当的核心问题在于权重方差设置错误会导致激活值分布逐层偏离正常范围，层层放大后训练举步维艰。BN 在每一层都进行一次标准化，相当于在初始化之后立即对激活值的分布进行修正，即便初始参数不够理想，也能被拉回合理范围，网络的容错性因此大幅提高，调试初始化参数的精力投入也相应减少。

除了直接的稳定性提升，BN 还带来一个有趣的副作用，它间接带来了一定的正则化效果。这个效果源于 BN 对 Mini-Batch 统计量的依赖。BN 在训练时使用当前 Mini-Batch 的均值 $\mu_B$ 和方差 $\sigma_B^2$ 进行标准化。这两个统计量是 Batch 中数据的样本估计，而非真实的总体统计量。不同的 Batch 包含不同的样本，因此 $\mu_B$ 和 $\sigma_B^2$ 随 Batch 变化而波动。这种波动意味着相同的输入 $x_i$，在不同的 Batch 中可能得到不同的标准化结果 $\hat{x}_i$。
...
需要注意的是，这种噪声仅在训练时存在。推理阶段使用全局统计量（所有训练 Batch 的滑动平均），标准化结果是确定的。这意味着训练时网络学习的是鲁棒特征，推理时这些特征被稳定地激活。这种训练有噪声、推理无噪声的设计，恰好符合正则化的目的。BN 的正则化效果在实践中表现为使用 BN 的网络往往泛化能力更好，训练损失和测试损失的差距更小。在一些情况下，BN 自带的正则化效果已经足够，甚至可以在一定程度上替代 Dropout。

尽管 BN 和 Dropout 都是改善训练的技术，两者的机制和效果有本质差异。从噪声的来源看，BN 的噪声来自 Batch 统计量的随机波动，噪声幅度取决于 Batch 内样本的分布差异，属于被动的、数据驱动的噪声。Dropout 的噪声则来自主动随机丢弃神经元，噪声强度由丢弃概率 $p$ 直接控制，可以人为调节。从对训练的影响看，两者的方向甚至相反。BN 通过标准化让梯度传播更顺畅，本质上是加速训练。Dropout 通过每次只激活部分网络来降低模型容量，反而可能拖慢训练的进度。这也自然导向了不同的适用范围，BN 已经成为 CNN 等深度网络的标准配置，几乎所有现代视觉网络都离不开它。Dropout 则更多地活跃在全连接层中，尤其是那些参数量庞大的分类器层。

现代深度网络（如 ResNet 及后续架构）的典型设计是卷积层后接 BN 和 ReLU，全连接层则视情况决定是否加 Dropout。这种设计背后有几个考量。BN 本身已经在训练过程中注入了足够的噪声，带来了可观的正则化效果，再加一层 Dropout 往往是锦上添花而非雪中送炭，边际收益有限。而且，卷积层天然不适合 Dropout，相邻像素之间高度相关，随机丢弃几个神经元，邻近的像素照样能把信息传过去，正则化效果大打折扣。更微妙的是，Dropout 在训练时会随机改变激活值的分布，这与 BN 努力维持分布稳定的目标恰好矛盾，可能让 BN 计算出的统计量本身也变得不稳定。

那么在实际中，网络已经用了 BN，还要不要加 Dropout？答案取决于数据。数据集较小、过拟合压力大时，在全连接层额外加一层 Dropout 是值得的。数据集足够大、BN 自带的正则化效果已经能兜住泛化能力时，则可以大方地省略 Dropout，让网络结构更简洁。
...
全局统计量本身并不会影响训练，它们都是为推理阶段准备的。推理阶段之所以必须依赖全局统计量而非 Batch 统计量，最直接的原因是推理最常见的场景是处理单个样本（Batch Size = 1）。当 Batch 只有一个样本时，均值就是样本本身、方差为零，标准化公式直接退化为 $\frac{x_1 - x_1}{\sqrt{0 + \epsilon}} = 0$，所有输入被抹成零，信息荡然无存。即便 Batch Size 大于 1，推理时使用 Batch 统计量也会导致输出随 Batch 的组成变化而波动，相同的输入在不同 Batch 中可能给出不同的预测，这种不确定性对于生产部署和调试来说是不可接受的。更何况生产环境中的部署场景五花八门，实时推理、批量处理、分布式部署的 Batch 组织方式各不相同，唯有使用固定的全局统计量，才能保证模型在所有场景下对相同输入给出相同输出，让测试和验证的结果具有一致性。
...
- **Batch Size 的依赖**。BN 的统计量估计质量直接取决于 Batch Size。当 Batch Size 很小（如 $m < 8$）时，统计量的方差很大，估计不稳定。极端情况下 Batch Size = 1，方差 $\sigma_B^2 = 0$，标准化完全失效。Batch Size 并非人为可随意调节，在显存受限、需要高分辨率图像训练等场景中，无法提高 Batch Size 是很现实的问题。
...
- **Layer Normalization**（LN）：完全不依赖 Batch 的标准化方案。LN 对单个样本的所有特征计算统计量：
    ...
    LN 的统计量来自单个样本内部，与 Batch Size 完全无关。这使得 LN 天然适用于 RNN 和 Transformer，每个时刻的隐藏状态可以独立标准化，训练和推理行为一致。LN 是 [Transformer 架构的默认归一化方案](../../language-models/architecture-basics/transformer-architecture.md#层归一化)。
...
- **Local Response Normalization**（LRN），这是 BN 出现之前主要在 CNN 中使用的一种通道间的归一化操作，灵感来自生物神经系统中的侧抑制现象，活跃的神经元抑制相邻神经元活动。设 $a_{x,y}^i$ 是位置 $(x, y)$、通道 $i$ 的激活值（ReLU 输出）。LRN 输出 $b_{x,y}^i$：

    $$b_{x,y}^i = \frac{a_{x,y}^i}{\left(k + \alpha \sum_{j=\max(0, i-n/2)}^{\min(N-1, i+n/2)} (a_{x,y}^j)^2\right)^\beta}$$

    其中，$\alpha$ 是缩放因子（默认 $\alpha = 10^{-4}$），$\beta$ 是指数（默认 $\beta = 0.75$）。分母中累加相邻通道的平方激活值，对高激活值的通道进行抑制。激活值越大，分母越大，输出相对减小。这鼓励了不同通道间的竞争性，增加了特征多样性。LRN 是 [AlexNet](../convolutional-neural-network/alexnet.md) 的标准配置，AlexNet 论文的实验表明 LRN 使 Top-1 错误率降低 1.4%，Top-5 错误率降低 1.2%。但后续研究表明，标准的 Batch Normalization（BN）在归一化方面比 LRN 更有效。现代 AlexNet 实现通常使用 BN 替代 LRN，或直接省略归一化。

不同归一化方案各有适用场景，选择时最现实的约束是 Batch Size。如果显存充裕、Batch Size 能稳定在 16 以上，BN 仍然是首选。一旦 Batch Size 受限，就要转向对 Batch 规模不敏感的 GN 或 LN。架构类型也直接决定了归属，CNN 体系下 BN 或 GN 是标准搭配，而 RNN 和 Transformer 天然适合 LN，因为序列长度不定、Batch 维度不可靠。此外，某些特殊任务有自己的偏好，比如风格迁移中 Instance Normalization 几乎是标配，因为它需要在单样本内部消除风格差异。这些方案并非互斥，在一些复杂架构中，不同部分可能使用不同的归一化，譬如 Transformer 的卷积部分用 GN、注意力部分用 LN，灵活组合反而是高级设计的标志。下表总结了各方案的特点和推荐用途。
