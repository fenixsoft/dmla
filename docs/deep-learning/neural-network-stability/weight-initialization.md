...
在介绍权重初始化方法前，我们先考虑如果不进行权重初始化，让神经元全部从相同的参数值（譬如零值）开始，训练会有什么结果？考虑一个简单的两层全连接网络，输入 $x$ 经过隐藏层 $h$ 到达输出 $y$（即 $h = \mathbf{W}_1 x$，$y = \mathbf{W}_2 h$），其中 $\mathbf{W}_1$ 是输入层到隐藏层的权重矩阵，$\mathbf{W}_2$ 是隐藏层到输出层的权重矩阵。如果我们把这两个矩阵都初始化为零（即 $\mathbf{W}_1 = 0, \quad \mathbf{W}_2 = 0$），那么在[前向传播](../neural-network-structure/forward-propagation.md#信号流动过程)阶段，就会发现隐藏层所有神经元的输出都是 $h_i = 0$（因为权重为零），输出层所有神经元的输出也都是 $y_i = 0$（因为 $h = 0$ 且权重为零）。这说明网络的表达能力完全退化，原本希望 100 个隐藏神经元来学习不同的特征，现在这 100 个神经元变成了 100 个复制品。

网络也无法通过训练来摆脱困境，因为在[反向传播](../neural-network-structure/backpropagation.md#反向传播)阶段问题同样严重。隐藏层权重的梯度为 $\frac{\partial L}{\partial \mathbf{W}_1} = \mathbf{W}_2^T \cdot \frac{\partial L}{\partial y} \cdot \mathbf{x}^T$，当 $\mathbf{W}_2 = 0$ 时，梯度直接为零，意味着参数无法更新，网络永远停留在初始状态。哪怕只初始化 $\mathbf{W}_1$ 为零，$\mathbf{W}_2$ 不为零，由于 $h = 0$，即使 $\mathbf{W}_2$ 的每个分量有不同值，也会使得误差传播到所有隐藏神经元时完全一致，隐藏层所有神经元仍然接收相同的梯度，更新后的权重也完全相同。这种所有神经元被迫学习相同的功能，网络的多层、多神经元结构变得毫无意义的现象被称为**对称性破坏问题**（Symmetry Breaking Problem）。全零初始化剥夺了神经元之间的差异性，使精心设计的网络结构退化成单个神经元。
...
- **参数过大**（譬如 $\sigma = 10$）：前向传播时，权重要乘以输入产生巨大的激活值。Sigmoid、tanh 这些激活函数收到 100 这样的输入，输出就很接近 1 了，激活值一直处于饱和边界上，反向传播时梯度接近零，没有传递什么参数更新信息。
- **参数过小**（譬如 $\sigma = 0.001$）：前向传播时，激活值逐层衰减，信号像回声一样越传越弱。反向传播时，梯度也逐层缩小，到前面几层时，也没有了参数更新信息，训练几乎停滞。

以上这些现象就是经典的[梯度消失](../neural-network-structure/activation-loss-functions.md#梯度消失与梯度爆炸)问题，由此可见，好的初始化需要在打破对称性，让每个神经元有独特的个性和保持信号强度，避免梯度消失，让误差信号能稳定传回输入层两个目标之间找到平衡。如何找到恰到好处的随机分布参数，让这两个看似矛盾的目标能够同时达成，正是 Xavier 初始化和 He 初始化要解决的问题。
...
当然，基于假设的约束，Xavier 初始化是存在明显的局限性的，一方面当输入远离 0 时，激活函数非线性特征显现，Xavier 初始化只能尽量让激活值落在线性区域，但无法完全避免饱和。另一方面更为严重，Xavier 初始化不适配 [ReLU 激活函数](../neural-network-structure/activation-loss-functions.md#relu-及其变体) ，ReLU 的特性与 Sigmoid 和 tanh 完全不同。ReLU 只保留正值，负值全部置零，这意味着约一半的激活值被杀死，信号强度减半。Xavier 的线性假设在 ReLU 上完全失效，如果用 Xavier 初始化 ReLU 网络，激活值会逐层衰减，深层网络的梯度几乎为零。针对 ReLU 的特殊性，需要一种新的初始化方法，这正是 He 初始化要解决的问题。
...
He 初始化的方差公式只考虑 $n_{in}$，而 Xavier 考虑 $n_{in} + n_{out}$ 的调和平均，这是因为 He 初始化更强调前向传播的信号补偿，ReLU 的稀疏性问题主要发生在前向传播阶段，反向传播时梯度只通过正值神经元传递，同样存在信号减半的问题，但 He 初始化优先保证前向传播的信号稳定性，因此仅对前向方向进行补偿。当 $n_{in} \approx n_{out}$ 时，He 初始化方差约为 Xavier 的两倍。
...
行文至此，我们讨论的都是权重初始化，但神经元的参数包括权重和偏置，偏置应该如何初始化？大多数情况下，偏置初始化为零是最安全的选择。偏置不参与信号强度的传递，权重矩阵乘以输入决定了信号如何流过网络，偏置只是在此基础上添加一个常数偏移，不改变信号的相对强度，因此零偏置不会像零权重那样导致对称性问题。更重要的是，对于 Sigmoid 和 tanh 这类在零点附近近似线性的激活函数，零偏置恰好让激活函数的输入 $z = \mathbf{W}x + b$ 近似等于 $\mathbf{W}x$（零均值），帮助激活值落在 Xavier 初始化所依赖的线性区域内。反过来说，如果偏置初始化为较大的正值或负值，Sigmoid 输出可能直接饱和到 1 或 0，梯度几乎为零，训练还没开始就已经陷入了梯度消失的泥潭。

但也有一些场景需要刻意给偏置赋非零值。在 ReLU 网络中，负输入会被直接置零，即使 He 初始化让加权求和结果的方差保持适中，仍然大约有一半的激活值会落在负半轴而被截断，导致网络初始输出过于稀疏。此时将偏置设为一个很小的正值（如 0.01），就可以把激活函数的输入整体向右推一点点，确保大部分神经元在初始阶段有非零输出，有效降低神经元过早死亡的风险。另一个经典的例子是 LSTM 的遗忘门偏置。遗忘门控制着前一时刻的记忆有多少被保留下来，输出接近 0 时几乎全部遗忘，接近 1 时几乎全部保留。如果遗忘门偏置初始化为零，Sigmoid 的输出初始大约在 0.5 左右，历史信息在训练初期就被无差别地砍掉一半。将遗忘门偏置设为 1 或更大的正值，使初始输出接近 1，让网络在训练早期倾向于保留更多历史信息，这对学习长期依赖关系至关重要。

## 本章小结

本章从为什么初始化至关重要出发，系统分析了权重初始化对深度网络训练的影响，并介绍了 Xavier 初始化和 He 初始化两种经典方法。初始化的挑战在于找到平衡点，全零初始化无法破坏对称性，使所有神经元学习相同功能，网络退化到单个神经元。随机初始化打破对称性，但方差选择不当会导致两个极端，方差太小，信号逐层衰减，训练缓慢，方差太大，激活值饱和，梯度消失。好的初始化需要在打破对称性和保持信号强度之间找到平衡。

初始化是训练的起点，决定了网络能否顺利起步。但训练的终点是能否收敛到最优，这里遇到的最大困难是过拟合，我们曾在线性模型部分讲解过通过 L1、L2 范数正则化来处理过拟合问题。下一章还将介绍 Dropout 正则化，通过随机丢弃神经元，迫使网络学习更鲁棒的特征，从另一个角度提升训练稳定性。
...
1. 解释为什么 Xavier 初始化采用 $\frac{1}{n_{in}}$ 和 $\frac{1}{n_{out}}$ 的调和平均 $\frac{2}{n_{in} + n_{out}}$，而非算术平均 $\frac{1}{2}\left(\frac{1}{n_{in}} + \frac{1}{n_{out}}\right)$。从梯度稳定性角度说明调和平均的优势。
    <details>
    <summary>参考答案</summary>
