...
当然，基于假设的约束，Xavier 初始化是存在明显的局限性的，一方面当输入远离 0 时，激活函数非线性特征显现，Xavier 初始化只能尽量让激活值落在线性区域，但无法完全避免饱和。另一方面更为严重，Xavier 初始化不适配 [ReLU 激活函数](../neural-network-structure/activation-loss-functions.md#relu-函数) ，ReLU 的特性与 Sigmoid 和 tanh 完全不同。ReLU 只保留正值，负值全部置零，这意味着约一半的激活值被杀死，信号强度减半。Xavier 的线性假设在 ReLU 上完全失效，如果用 Xavier 初始化 ReLU 网络，激活值会逐层衰减，深层网络的梯度几乎为零。针对 ReLU 的特殊性，需要一种新的初始化方法，这正是 He 初始化要解决的问题。
