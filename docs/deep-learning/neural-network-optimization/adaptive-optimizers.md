...
前面介绍的优化器的共同特征是为每个参数分配独立的学习率，通过梯度历史来调整更新步长。这种逐参数自适应的策略取得了巨大成功，但是神经网络参数其实并非的独立标量，参数之间是存在关联关系的。神经网络隐藏层的权重天然是二维矩阵，行对应对输入特征的变换，列对应对输出神经元的组合。将矩阵展平为向量再逐元素调整学习率，等于主动放弃了矩阵的几何信息。2024 年 12 月，凯勒·乔丹（Keller Jordan）在博客文章中提出了 Muon 优化器，不走逐参数自适应学习率的传统路径，通过正交化动量更新来利用矩阵的几何结构。
