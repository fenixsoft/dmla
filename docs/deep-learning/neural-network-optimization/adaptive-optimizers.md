...
前面介绍的优化器的共同特征是为每个参数分配独立的学习率，通过梯度历史来调整更新步长。这种逐参数自适应的策略取得了巨大成功，不过，神经网络参数其实并非的独立标量，参数之间是存在关联关系的。神经网络隐藏层的权重天然是二维矩阵，行对应对输入特征的变换，列对应对输出神经元的组合。将矩阵展平为向量再逐元素调整学习率，等于主动放弃了矩阵的几何信息。2024 年 12 月，凯勒·乔丹（Keller Jordan）在博客文章中提出了 Muon 优化器，不走逐参数自适应学习率的传统路径，通过正交化动量更新来利用矩阵的几何结构。

Muon 的名称源自"动量经牛顿-舒尔茨正交化"这个拗口复杂的名字（**M**oment**u**m **O**rthogonalized by **N**ewton-Schulz），它的具体步骤却出人意料地简单，只有两步：
...
牛顿-舒尔茨迭代利用了一个数学事实：如果把一个精心构造的多项式函数反复作用在矩阵上，这个函数会作用在矩阵的"强度"部分而非"方向"部分，逐步把所有强度推向 1，最终逼近正交化的结果。整个过程只需要矩阵乘法，不需要显式地分解矩阵。乔丹团队选了一个五次多项式，并在大量实验中调优了多项式的三个系数，使得仅需五轮迭代就能在 BF16 精度下稳定逼近正交化的效果。迭代开始前，先把矩阵整体缩放到单位长度，确保所有强度落在 0 到 1 之间。然后每轮迭代用多项式推动一轮，那些初始就很小的强度会被快速拉升，逐渐逼近 1。五轮之后，矩阵已经非常接近正交化的结果。与直接做奇异值分解（太慢）和另一种称为耦合牛顿迭代的方法（需要 FP32 精度，在 GPU 上效率低）相比，牛顿-舒尔茨迭代可以在现代 GPU 原生的 BF16 精度下运行，速度快、数值稳定，这才是它真正能嵌入训练循环的关键。

走完这两步后，Muon 将正交化后的动量作为最终更新，乘以全局学习率作用到权重上。整个过程可以概括为动量累积提供平滑的方向，正交化拉平所有方向的强度，学习率控制整体步长。不过既然 Muon 利用了矩阵的二维几何关系，它就仅能适用于神经网络隐藏层的二维权重矩阵。对于标量参数（如偏置）、向量参数（如层归一化的缩放因子）以及输入/输出嵌入层，仍需使用 AdamW 等标准优化器。对于四维卷积核，可以将最后三个维度展平为二维矩阵后使用 Muon。这种混合优化器策略（隐藏层用 Muon、其余参数用 AdamW）是 2026 年语言模型使用的主流方式。

在性能方面，Muon 在多项基准测试中表现出色。它将 CIFAR-10 达到 94% 准确率的训练时间压缩了约 21%。在 NanoGPT 速度竞赛（FineWeb 数据集）中，Muon 将训练速度提升了 1.35 倍。使用 8 张 H100 训练 1.5B 参数的 Transformer 达到 GPT-2 XL 级别的性能，Muon 仅需 10 小时，而 AdamW 则需要 13.3 小时。此后，Moonlight 16B 混合专家模型（5.7T tokens 训练数据）等大规模项目进一步验证了 Muon 在工业级场景下的可行性。

Muon 的计算开销和内存开销都很低。在典型的语言模型训练中（如 Llama 405B，模型维度 16384，批大小 16M tokens），五步牛顿-舒尔茨迭代的额外浮点运算占比不到 1%。在内存方面，Muon 仅维护一个动量缓冲区（与 SGD 动量相同），相比 AdamW 直接节省了约一半的优化器状态内存，大幅降低了训练环境的资源门槛。
...
- 步骤一，首先了解各个优化器的特点：
    ...
    | AdamW | Adam+解耦权重衰减 | 正则化稳定 | 资源门槛要求较高 | 默认选择 |
    | Muon | 动量 + Newton-Schulz正交化 | 利用矩阵结构、内存省 | 仅适用于2D矩阵参数 | Transformer隐藏层、大规模训练 |
...
- 步骤二，根据任务特点选择优化器：
    ...
    | 大规模语言模型训练 | Muon + AdamW | 隐藏层用Muon提速，其余参数用AdamW |
