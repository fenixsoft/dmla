...
Seq2Seq 要解决的问题是将变长输入序列映射到变长输出序列。输入和输出序列长度不相等的情况现实中非常普遍，譬如翻译 "Good good study, day day up" 只有六个英文单词，对应的中文"好好学习，天天向上"却是八个字；一篇五百字的新闻文章，摘要可能只有五十个字；用户提出一个简短的问题，回答却可能需要解释一大段背景。以前 RNN 为了满足输入序列与输出序列等长的约束，必须[强行对齐](lstm-gru.md#训练技巧与最佳实践)（如把短序列填充到相同长度），导致输出包含大量无意义的空白位置，或者丢失本应生成的关键内容。
