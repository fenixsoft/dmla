...
在开始实验之前，请确保已[挂载数据目录](../../appendixes/sandbox.md#数据管理)并下载好 chinese-poetry 古诗词数据集，你可以通过 `DMLA-CLI` 工具自动完成该工作：
...
- **数据清洗**：古诗数据中包含标题、作者、注释等元信息，训练时只需保留诗词正文。同时需要过滤掉残缺不全的诗句，以及过短或过长的作品（过短信息量不足，过长训练效率低）。
- **序列构建**：LSTM 训练采用[教师强制](seq2seq.md#计划采样)（Teacher Forcing）模式，输入序列为目标序列去掉最后一个字符，目标序列为原序列去掉第一个字符。譬如诗句"床前明月光"，输入为"床前明月"，目标为"前明月光"。模型学习根据前文预测后文。

下面代码不会产生缓存或者其他中间结果，因此无需手工执行，会在第三阶段模型训练中被自动调用。
...
- **批处理与序列打包**：由于诗词长度不一，使用填充（Padding）将同一批次内的序列对齐到相同长度。为提高效率，减少每批内的填充量。
...
LSTM 语言模型在古诗词生成任务上的训练结果需要从多个维度来审视。

训练损失是最直接的量化信号。交叉熵损失越低，说明模型对训练数据的拟合越好，但损失过低反而值得警惕。它可能意味着模型只是在机械地背诵训练数据，而非真正学会了诗词的生成规律。理想的损失曲线应该先快速下降，然后逐渐趋于平稳，在收敛与泛化之间找到一个平衡点。

生成的诗词质量则很难用单一数值来评判，很大程度上依赖人的主观感受。一首好的生成作品，首先语义上要通顺，至少读起来像一句完整的话，而不是随机字符的堆砌。其次，即使模型没有显式地学过格律规则，在大量诗词数据中浸泡之后，它往往会自发地捕捉到一些基本的韵律模式，譬如五言、七言的节奏感，或是句尾的押韵倾向。更高阶的要求是意境，能让人读后觉得有点味道，而不只是平淡无奇的字句拼接。

温度参数是控制生成多样性的关键旋钮。低温（0.3-0.5）下模型倾向于选择最稳妥的高概率字符，生成结果稳定、保守，读起来像是训练集中最常见的表达方式，但也容易显得千篇一律。中温（0.7-0.9）在稳定性和多样性之间取得了不错的平衡，是日常实验中最推荐的默认选择。高温（1.0-1.5）则彻底放飞，模型敢于尝试小概率的字符组合，不时冒出令人惊喜的创意，但也随时可能跌入不通顺的乱码。

如果希望进一步改善生成质量，工程上还有几条可行的路径。数据层面，可以引入更多诗词数据，或者用现代诗歌来增加表达的多样性。模型层面，增加 LSTM 的层数或隐藏维度能提升容量，让网络有更多空间去学习细腻的语言模式。也可以在生成过程中加入平仄、押韵等显式的格律约束，让输出自动对齐古典诗词的形式要求。更激进的做法是直接换用 Transformer 架构，在序列建模任务上，Transformer 通常比 LSTM 有更强的表达能力和更长的记忆跨度。
