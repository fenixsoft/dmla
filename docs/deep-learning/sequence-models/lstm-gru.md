...
- **计算资源受限场景**：Transformer 的自注意力机制计算复杂度为 $O(n^2)$，序列长度翻倍，计算量增长四倍。LSTM/GRU 的复杂度为 $O(n)$，线性增长。在边缘设备、嵌入式系统或移动应用中，LSTM/GRU 的参数量和计算开销远低于 Transformer，是更实际的选择。譬如一个 128 维隐藏状态的单层 GRU 仅有约 33K 参数，而同等规模的轻量级 Transformer 参数量通常此数百倍以上。
