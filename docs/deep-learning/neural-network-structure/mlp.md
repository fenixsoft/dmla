...
- **深度网络（$n-1$ 层）**：每层仅需 $O(1)$ 个神经元计算两个中间结果的乘积。具体构造如下：第 1 层计算 $x_1 \cdot x_2$、$x_3 \cdot x_4$ 等；第 2 层将上层结果两两相乘；以此类推，经过 $n-1$ 层后得到最终乘积。每层仅需常数个神经元，总参数量为 $O(n)$。譬如，$n=20$ 时，深度网络仅需约 $20$ 层、每层少量神经元，总参数量约 $O(20)$，与宽度网络的百万级参数形成鲜明对比。
