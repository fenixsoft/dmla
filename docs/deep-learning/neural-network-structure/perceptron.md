...
感知机是一个单层神经网络，由输入层和输出层组成，没有隐藏层。设输入向量为 $\mathbf{x} = (x_1, x_2, \ldots, x_n)^T$，权重向量为 $\mathbf{w} = (w_1, w_2, \ldots, w_n)^T$，偏置（bias）为 $b$。其基本结构如下：
...
偏置 $b$ 可以理解为阈值 $\theta$ 的负值。在上一章的 M-P 模型中，阈值条件是 $\sum w_i x_i \geq \theta$；若将阈值移到左侧，变为 $\sum w_i x_i - \theta \geq 0$，则 $b = -\theta$。这种形式在数学处理上更为方便，因为决策边界可以统一写成 $\mathbf{w}^T \mathbf{x} + b = 0$。

### 与 M-P 模型的关系

感知机继承了 M-P 模型的核心思想：加权求和、阈值决策、二值输出。两者的关键区别在于**学习能力**：

- M-P 模型的权重和阈值需要人工设定，模型本身没有学习能力。
- 感知机引入了学习算法，能够根据训练数据自动调整权重和偏置。

这一进步源于 Hebb 学习规则的思想：权重可以根据神经元的活动进行调整。感知机将 Hebb 规则的"相关性学习"发展为"错误驱动学习" —— 只有当预测错误时才调整权重，调整的方向是减少预测错误。
...
这种表示方式将偏置视为一个"常数输入 1"对应的权重，简化了数学推导。后续讨论中，若无特殊说明，我们使用增广向量形式，并省略"增广"标记，直接记为 $\mathbf{w}$ 和 $\mathbf{x}$。
