...
在前三章中，我们逐步构建了神经网络的理论框架。从生物神经元到 M-P 模型，从单层感知机到多层感知机，从线性决策边界到非线性表达能力。现在，我们将深入神经网络的计算机制 —— **前向传播**（Forward Propagation）。前向传播的概念可以追溯到 1943 年。麦卡洛克和皮茨提出 M-P 模型时，所描述的神经元接收输入信号、加权求和、阈值处理后输出信号的过程，正是前向传播的雏形。如今，前向传播特指神经网络中信号从输入层经各层神经元逐层传递至输出层的过程。本章将介绍信号流动过程、矩阵形式推导、计算图概念、批量计算与效率优化，并通过实验比较前向传播在 CPU 和 GPU 上的速度差异。

## 信号流动过程

前文介绍感知机和多层感知机时，都是以单个神经元的角度去看待信号流动的，神经元接收多个输入信号 $\mathbf{x} = (x_1, x_2, \ldots, x_n)^T$，对每个输入信号乘以对应的权重并加上偏置值，得到线性组合的结果 $z = \sum_{i=1}^{n} w_i x_i + b = \mathbf{w}^T \mathbf{x} + b$，再通过激活函数 $f$ 变换，产生神经元输出 $a = f(z)$。这里"**输入 → 加权求和 → 激活变换**"的三步流程就构成了神经元的基本计算过程。
...
矩阵形式将前向传播表达为紧凑的数学公式，现在我们把关注点从数学理论转移到工程实践，探讨深度学习框架如何管理复杂的计算流程。现代深度学习框架能够高效处理神经网络大量运算工作的关键是**计算图**（Computational Graph），它不仅是理解和调试神经网络的有力工具，更是[自动微分](../../appendixes/numpy/calculus-numpy.md#自动微分)（Automatic Differentiation）的基础，为反向传播提供技术支撑。

计算图是一种表示计算过程的图形化方法，图中的节点表示运算（如加法、乘法、激活函数），边表示数据流动，将复杂的计算过程分解为基本运算的组合。神经网络的前向传播可以自然地表示为计算图，每个神经元包含两个节点，线性组合节点（$\mathbf{z} = \mathbf{W}\mathbf{x} + \mathbf{b}$）和激活节点（$\mathbf{a} = f(\mathbf{z})$）。多层网络将这些节点按层连接，形成完整的计算图。以单层感知机为例，其计算图结构如下：
...
图中每个节点代表一个运算，如权重乘法（$\mathbf{W}·\mathbf{x}$）、偏置加法（$z$）、激活变换（$a$）。数据沿着边从输入节点流向输出节点，形成完整的计算链路。对于多层网络，计算图是多个单层计算图的串联，如下图所示：
...
在现代计算机体系中，计算图带来的第一个好处是让计算过程变得可视且透明。数据如何从输入逐层流向输出、每一步执行了什么运算，全部展现在图上，调试神经网络时可以直接顺着图的拓扑结构定位计算错误或理解中间结果。但计算图最重要的贡献远不止可视化，而是它为自动微分提供了数据结构层面的支撑。反向传播需要计算每一层的梯度，手工推导这些梯度公式既繁琐又容易出错，计算图将复合函数分解为节点和边的有向无环图之后，梯度就可以沿着图的逆向边自动传播，链式法则的每一步对应图中一条边的局部导数，逐段累乘即可得到任意节点对任意参数的梯度。TensorFlow、PyTorch 等框架之所以能让用户只写前向代码就自动获得梯度，底层正是靠这套计算图机制在运转。此外，计算图天然鼓励模块化设计，加法、乘法、激活函数，每一种基本运算都是独立的积木块，用户可以自由组合这些积木来搭建任意复杂的网络结构，现代框架提供的丰富算子库便是最好的例证。有了计算图的依赖关系，框架还能在背后做更深层的优化，如算子融合将连续的多个小运算合并为一次大运算，减少内存读写次数。无依赖的节点可以被识别出来并行执行，大幅提升训练效率，这些优化在现代框架中早已是标配。

### 静态图与动态图

深度学习框架中，计算图有静态图和动态图两种构建方式。理解两者的区别，有助于理解不同框架（TensorFlow vs PyTorch）的设计理念。
...
打个生活的比方，静态图就像预制菜，工厂预先设计好配方、准备好材料，用户只需按流程执行，效率高但无法定制。动态图就像现炒现做，厨师根据实际情况实时调整，灵活但计算效率相对较低。现代框架逐渐融合两种方式的优点，TensorFlow 2.x 支持动态图模式（Eager Execution），但可以通过 `tf.function` 转换为静态图进行优化。PyTorch 默认动态图，但支持 JIT 编译将动态图优化为静态图。这种融合让用户在开发时享受动态图的灵活性，在生产部署时享受静态图的高效性。

## 批量计算与效率优化

矩阵形式为**批量处理**（Batch Processing）奠定了基础，批量处理不仅仅是单纯的数学技巧，更是神经网络训练效率的核心。神经网络训练通常涉及大量样本，以图像分类为例，MNIST 数据集有 6 万样本、CIFAR-10 数据集有 5 万样本、ImageNet 数据集有 128 万样本，而语言模型的文本分类中样本数量更是以亿为计数单位。由此可见，批量处理是优化计算效率必不可少的手段，它将多个样本合并为一个矩阵，一次性完成计算。设批量大小为 $B$（称为 Batch Size），前向传播一次处理 $B$ 个样本，输出 $B$ 个预测结果，反向传播则基于这 $B$ 个样本的梯度平均值来更新权重。

批量处理最直接的价值在于计算效率。现代 GPU 拥有数千到上万个计算核心，譬如 NVIDIA A100 就有 6912 个 CUDA 核心。这些核心的设计初衷就是高效执行矩阵运算，一次处理 $B$ 个样本的时间远小于逐个处理 $B$ 次的时间之和，就像卡车一趟运完一千块砖，远比工人来回搬运一千趟快得多。同时，批量处理还带来了梯度质量的提升。单个样本的梯度就像盲人摸象，噪声大、方向飘忽不定，训练过程来回震荡。将 $B$ 个样本的梯度取平均之后，噪声被平滑，方向变得稳定，训练也自然更加平稳。此外，批量处理对内存带宽的利用也更为友好。GPU 计算核心的速度极快，但数据从内存传输到核心的速度相对较慢，一次搬运 $B$ 个样本可以减少数据传输的次数，让计算核心保持持续运转而不是空转等待数据。因此，批量大小 $B$ 是神经网络中需要精心权衡的重要超参数，必须在计算效率、梯度稳定性和显存占用量三者之间找到平衡点。下表列出了常见的批量大小范围和选择的原则：
...
1. **矩阵运算优化**：利用 GPU 加速矩阵乘法（如 CUDA、cuBLAS），在 CPU 上计算一个 $n \times n$ 矩阵乘法需要 $O(n^3)$ 时间，GPU 可以利用数千个 GPU 核心并行计算，速度提升数十倍甚至上百倍。
2. **算子融合**：将线性组合和激活变换合并为单一操作，减少中间结果的存储和传输。传统实现先计算 $\mathbf{z}^l = \mathbf{W}^l \mathbf{a}^{l-1} + \mathbf{b}^l$，存储 $\mathbf{z}^l$，再计算 $\mathbf{a}^l = f(\mathbf{z}^l)$。融合实现直接计算 $\mathbf{a}^l = f(\mathbf{W}^l \mathbf{a}^{l-1} + \mathbf{b}^l)$，省去存储 $\mathbf{z}^l$ 的步骤，减少内存访问。更进一步，可以将多个连续运算合并为一个复合运算，减少计算图节点数量，降低执行开销。譬如将矩阵乘法 → 偏置加法 → 激活函数三个节点合并为单一节点，减少中间结果的存储和传输，减少函数调用次数。
...
从运行结果可以看出，由于有内存到显存拷贝等启动成本，在规模很小的神经网络中，GPU 并没有效率优势，但随着网络规模增大，GPU 的优势会迅速展现出来。
