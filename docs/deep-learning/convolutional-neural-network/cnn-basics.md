...
学习 CNN 的核心价值在于理解一个更宏观的设计哲学：好的模型不是靠堆参数，而是靠正确的假设。全连接层将图像视为扁平的像素向量，从一开始就丢弃了空间结构，这意味着它必须从头学习相邻像素之间存在关联这本应由架构保证的事实。卷积层则反其道而行，它假设空间关系天然重要，因此用极小的参数量就能捕捉到有意义的模式。这种思路在深度学习的设计中还会反复出现，与其让模型从零开始，不如将领域知识编码为结构约束。
