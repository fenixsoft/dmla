...
$C_{in}$ 和 $C_{out}$ 没有**数值上的约束关系**，即输出通道数可以大于、等于或小于输入通道数。但两者存在**操作层面的绑定关系**：每个卷积核必须覆盖所有输入通道，因此单个卷积核的空间尺寸是 $k \times k$，而完整尺寸是 $k \times k \times C_{in}$。这个三维结构确保每个输出值融合了所有输入通道的信息。
...
$C_{out}$ 是卷积层的核心设计参数，其选择遵循以下原则：

**通道数递增设计**是 CNN 的普遍规律，即浅层通道少、深层通道多。典型做法是每经过一次池化（空间尺寸减半），下次卷积时通道数翻倍。以 VGG-16 为例，输入 $224 \times 224 \times 3$，第一层输出 $224 \times 224 \times 64$，池化后变为 $112 \times 112 \times 64$，下一层卷积输出 $112 \times 112 \times 128$，再次池化后 $56 \times 56 \times 128$，后续继续翻倍至 256、512。这种设计的合理性在于：

- **浅层特征简单**：边缘、纹理等基础特征种类有限，64 个通道足以表示不同方向、不同强度的边缘模式
- **深层特征复杂**：物体部件的组合方式多样，需要更多通道捕捉不同的组合模式
- **信息补偿**：空间尺寸递减时，增加通道数可以补偿信息损失，保持总体信息容量

**任务需求**也影响通道数选择。分类任务末端通常通道数较少（如 ResNet 末端的 512 通道），因为最终只需输出类别概率；检测和分割任务需要保留更多空间细节，末端通道数可能达到 1024 或更多。

### 输出通道的作用

每个输出通道对应一个**独立的特征检测器**。$C_{out}=64$ 意味着该层能学习检测 64 种不同的特征模式。不同卷积核通过训练自动分化，可能学到：

- 不同方向的边缘（水平、垂直、45 度、135 度）
- 不同尺度的纹理（细粒纹理、粗块纹理）
- 颜色组合模式（红绿对比、蓝黄对比）
- 更复杂的局部结构（角点、圆弧、交叉点）

这些特征检测器并非人工设计，而是网络根据训练数据自动学习得到的。同一层的不同卷积核在训练过程中会自然分化，各自专注于不同的特征类型，避免冗余。这种"让网络自己学特征"的方式正是 CNN 的核心优势，远超传统图像处理中手工设计卷积核的局限。

除卷积核设计外，卷积操作还具有**步长**和**填充**两个超参数，共同决定输出特征图的尺寸。**步长**（Stride）是卷积核每次滑动的距离。步长 $s=1$ 时，卷积核逐位置滑动（默认方式）；步长 $s=2$ 时，卷积核跳过一个位置滑动。步长越大，输出尺寸越小，特征图越压缩。**填充**（Padding）是在输入图像边缘添加额外像素（通常填零）。无填充（Valid Padding）时，输出尺寸小于输入；等保填充（Same Padding）时，输出尺寸与输入相同（步长为 1 时）。填充的作用是保留边缘信息，不加填充时，边缘像素只被卷积核覆盖一次，而中心像素被覆盖多次，边缘信息容易被忽略。设输入尺寸 $n \times n$，卷积核尺寸 $k \times k$，步长 $s$，填充 $p$，则输出尺寸为：
