...
#### 方案 D：LMDB + 自适应 DataLoader（推荐方案）

**实现方式**：使用 LMDB 键值存储 + PyTorch DataLoader。**根据宿主系统自动选择最优配置**：

| 宿主系统 | DataLoader 配置 | 性能 |
|---------|----------------|------|
| **Windows Docker Desktop** | 单线程 (num_workers=0) | ~200ms/batch，GPU利用率 ~1-3% |
| **Linux Docker** | 多线程 (num_workers=4) | ~40-50ms/batch，GPU利用率 ~40-60% |

**技术要点**：
- LMDB 数据库存储预处理结果（mmap 零拷贝读取）
- 自动检测宿主系统（通过 `/proc/version`）
- Windows: 单线程 DataLoader（规避 shm 问题）
- Linux: 多线程 DataLoader（shm 配置可用）
- 数据增强在 CPU 上执行（RandomFlip、Crop）
- Normalize 在 GPU 上批量执行
- 跨平台兼容：同一代码在 Windows/Linux 均可运行
- 安装：`pip install lmdb`

**适用场景**：教学实验、生产环境、跨平台需求

::: warning Windows Docker shm 限制
Windows Docker Desktop 的 shm 实现与 Linux 不同，PyTorch DataLoader 多线程会报错：
```
RuntimeError: unable to allocate shared memory(shm)
```

方案 D 会自动检测操作系统，在 Windows 上使用单线程模式规避此问题。
:::
...
- **Linux Docker**: `num_workers=4`，4 个子进程并行读取数据
- **Windows Docker**: `num_workers=0`，单线程读取（规避 shm 问题）
- 主进程收集 batch 后传输到 GPU

**Windows Docker shm 问题**：
- PyTorch DataLoader 多线程需要共享内存（shm）
- Windows Docker Desktop 的 shm 实现不兼容 PyTorch 的 `_new_shared` 函数
- 错误：`RuntimeError: unable to allocate shared memory(shm)`
- 解决方案：自动检测操作系统，Windows 使用单线程

**性能对比**：
| 环境 | num_workers | 128 batch 耗时 | GPU 利用率 |
|------|------------|---------------|-----------|
| Windows Docker | 0（单线程） | ~200ms | ~1-3% |
| Linux Docker | 4（多线程） | ~40-50ms | ~40-60% |
...
本阶段执行完整的训练流程，采用方案 D（LMDB 存储）策略。

::: warning Windows Docker shm 限制
**Windows Docker Desktop 的 shm 问题**：
- Windows Docker 的 shm 实现与 Linux 不同
- PyTorch DataLoader 多线程 (`num_workers > 0`) 在 Windows Docker 内会报错
- 错误：`RuntimeError: unable to allocate shared memory(shm)`

**解决方案**：根据操作系统自动选择：
- **Windows Docker**: 使用单线程 DataLoader (`num_workers=0`)
- **Linux Docker**: 使用多线程 DataLoader (`num_workers=4`)
:::

**训练关键点：**

1. **OS 检测：** 通过 `/proc/version` 检测宿主系统
2. **自适应 DataLoader：** Windows 单线程，Linux 多线程
3. **LMDB 读取：** mmap 零拷贝读取
...
def detect_host_os():
    ...
    通过 /proc/version 内容判断：
    - Windows Docker Desktop: 包含 "Microsoft" 或 "WSL"
    - Linux Docker: 不包含上述关键字
    """
    try:
...
if host_os == 'windows':
    print("[环境检测] Windows Docker Desktop 模式：使用单线程 DataLoader")
    print("[环境检测] 预期性能：~200ms/batch，GPU 利用率 ~1-3%")
else:
    print("[环境检测] Linux Docker 模式：使用多线程 DataLoader")
    print("[环境检测] 预期性能：~40-50ms/batch，GPU 利用率 ~40-60%")
...
# ==================== 根据操作系统选择 DataLoader 配置 ====================
progress.update(30, message="创建 DataLoader...")

# Windows: 单线程（shm 不可用）
# Linux: 多线程（shm 已通过 CLI 配置）
num_workers = 0 if host_os == 'windows' else 4
...
progress.reset(total_steps=num_epochs * total_batches, description=f"训练 AlexNet（{host_os} 模式）")
current_batch = 0
best_acc = 0.0

print(f"[DEBUG] 开始训练: {num_epochs} epochs, 每 epoch {total_batches} batches")
print(f"[DEBUG] DataLoader: {num_workers} workers ({host_os} 模式)")
...
| 指标 | 方案 A | 方案 B | 方案 D (Windows) | 方案 D (Linux) | 原方案 |
|------|-------|-------|-----------------|---------------|--------|
| 内存占用 | ~2 GB | ~4 GB | **~2 GB** | **~2 GB** | ~67 GB |
| 磁盘缓存 | 250 MB | 600 MB | **600 MB** | **600 MB** | 60 GB |
| GPU 利用率 | ~15% | ~40-60% | **~1-3%** | **~40-60%** | ~80% |
| 每 epoch 时间 | ~30 分钟 | ~10-15 分钟 | **~2-3 分钟** | **~40-60 秒** | ~5 分钟 |
| DataLoader workers | 0 | 0 | **0** | **4** | 4 |
| 跨平台兼容 | ✅ | ✅ | **✅** | **✅** | ✅ |

**方案选择建议**：
- **Windows Docker**：方案 D 自动使用单线程 DataLoader
- **Linux Docker**：方案 D 自动使用多线程 DataLoader —— **更高性能**
- **内存 < 16 GB**：使用方案 A（实时预处理）
- **内存 16-32 GB**：使用方案 B（最小缓存）
- **内存 > 64 GB**：使用原方案（全量缓存）

**方案 D 的优势**：
- 环境自适应：自动检测宿主系统，选择最优配置
...
| 模型训练 | LMDB + DataLoader (自适应) | `timeout=unlimited` | Windows: ~200ms/batch, Linux: ~40-50ms/batch | **~2 GB** |
| 模型推理 | 加载模型预测 | 常规 | - | ~200 MB |

**性能优化要点（方案 D）**：

1. **LMDB 高效存储**：mmap 零拷贝读取，避免大量小文件 I/O
2. **OS 自适应配置**：自动检测 Windows/Linux，选择最优 DataLoader 配置
3. **Windows 单线程**：规避 Docker shm 问题，稳定运行
4. **Linux 多线程**：`num_workers=4` 利用多核 CPU 并行解码
5. **跨平台兼容**：同一代码在 Windows/Linux 均可运行
...
| **D: Windows** | **2GB** | **600MB** | **~1-3%** | **~2-3分钟** | **Windows Docker** |
| **D: Linux** | **2GB** | **600MB** | **~40-60%** | **~40-60秒** | **Linux Docker** |
