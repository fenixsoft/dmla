...
3. **DataLoader 配置：** `num_workers=0` 单线程（Docker shm 默认 64MB 限制）
...
progress.update(0, message="正在检查 LMDB 缓存...")
...
if not os.path.exists(manifest_path) or not os.path.exists(train_lmdb_path):
    ...
    print(f"预计内存占用: ~4 GB（LMDB mmap 读取）")
...
# 创建 DataLoader（LMDB 支持多线程读取）
progress.update(30, message="创建 DataLoader...")
train_loader = DataLoader(
    train_dataset,
    batch_size=128,
    shuffle=True,
    num_workers=4,           # LMDB 支持多线程（无锁读取）
...
print(f"[DEBUG] DataLoader 配置: num_workers=4, pin_memory=True")
...
def train_one_epoch(model, train_loader, criterion, optimizer, device, epoch, progress, current_batch, perf_log):
    ...
    correct = 0
    total = 0
    prev_batch_end = time.time()  # 记录上一个 batch 结束时间
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if batch_idx % 10 == 0:
            perf_log.write(f"{batch_idx},{data_load_time*1000:.1f},{gpu_transfer_time*1000:.1f},{normalize_time*1000:.1f},{forward_time*1000:.1f},{backward_time*1000:.1f},{optimizer_time*1000:.1f},{batch_total*1000:.1f}\n")
        ...
        # 记录当前 batch 结束时间（用于下一个 batch 的数据加载计时）
...
3. **单线程 DataLoader**：`num_workers=0`（Docker 容器 shm 限制）
...
**预处理缓存（方案 B）**：
- `/data/cache/preprocessing/tiny-imagenet-224-minimal/train/*.JPEG` - 训练集 Resize 后的 JPEG（200 个类别目录）
- `/data/cache/preprocessing/tiny-imagenet-224-minimal/val/*.JPEG` - 验证集 Resize 后的 JPEG（10000 张）
- `/data/cache/preprocessing/tiny-imagenet-224-minimal/manifest.json` - 缓存清单（标签映射）
