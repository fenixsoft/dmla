...
- **全参数微调**需要为全部 65B 参数维护权重、梯度和优化器状态，约 744 GB 显存，至少需要 5 张 A100 80GB 才能容纳：
    ...
    | 优化器（Adam FP32） | 65B × 8 字节 | 484 GB |
    | 激活值（[梯度检查点](../../deep-learning/neural-network-structure/backpropagation.md#计算复杂度分析)） | 经验估算 | ~18 GB |
    | **合计** | | **~744 GB** |

- **LoRA**冻结原始权重，仅训练低秩旁路。假设对每层注意力的 Q 和 V 投影添加秩 $r=16$ 的 LoRA 旁路，每层 LoRA 参数量为 $4 \times 8192 \times 16 = 524{,}288$，80 层合计约 41.9M 参数，仅占模型总参数的 0.06%。显存需求从 744 GB 降至约 140 GB，节省 81%，2 张 A100 80GB 即可完成训练。具体显存占用如下：
