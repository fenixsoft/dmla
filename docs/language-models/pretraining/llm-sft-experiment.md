...
按序列长度 768，批大小 16（梯度累积 × 2，等效批大小 32），2 个 epoch，约需 6 GB 显存可运行，用 5080 GPU 训练约 15 分钟。
...
| 梯度累积 | 1（batch_size=16） | 2（batch_size=16，等效 32） | 原版直接用 batch_size=16 每步更新一次。SFT 的序列长度 768 比预训练的 512 更长，显存占用更高，batch_size=32 在 8G 显存上会 OOM。用 batch_size=16 + accumulation_steps=2 保持等效批大小 32，显存约 5.7 GB，7.6 GB 足够 |
