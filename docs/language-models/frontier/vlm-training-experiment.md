...
从运行结果可以看出，64M 参数的 VLM 模型已经能够识别图像中的部分元素（如海滩、城市街道、舞者等），并生成结构完整的中文描述。但受限于参数规模，模型对图像细节的理解存在明显偏差，例如将熊猫识别为"狗和羊群"、将自行车与花朵的场景描述为"汽车和浴缸"。这些偏差源于模型有限的参数规模，在更大规模的 VLM（如 LLaVA-1.5、Qwen-VL）中会显著改善。
