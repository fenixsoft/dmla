...
在[预训练实验](../architecture-basics/llm-pretrain-experiment.md)中，我们训练了一个能够理解和生成文本的纯语言模型。但人类感知世界的方式远不止语言，视觉是我们获取信息最直接的途径。本次实验将在预训练语言模型的基础上，为其安装一双眼睛——视觉编码器，让模型能够同时理解图像和文本，完成图像描述、视觉问答等多模态任务。与前面基于 MiniMind 的预训练实验类似，本次实验的数据集来自 [MiniMind-V](https://github.com/jingyaogong/minimind-v)。

## 实验准备

在开始实验之前，请确保已完成[预训练实验](../architecture-basics/llm-pretrain-experiment.md)，并下载好视觉训练数据。通过 `DMLA-CLI` 工具完成数据下载：
...
> **数据说明**：对于 64M 的 VLM 实验而言，原有的数据规模偏大，本实验从原始 ALLaVA-4V 数据集中随机抽取了 20% 的样本。由于随机采样打破了原始文件中相邻图片的压缩局部性，文件体积不会严格按 20% 线性缩小。如需完整数据集（原始 417 万条样本），可从 [MiniMind-V 官方仓库](https://github.com/jingyaogong/minimind-v) 获取。
...
if os.path.exists(data_dir):
    ...
    print("数据目录未下载，请运行 'dmla data' 下载 MiniMind Vision 数据集")
```

## 第一阶段：视觉编码器与投影层

视觉语言模型的思路是将图像转化为语言模型能理解的视觉 token，然后让语言模型像处理文本 token 一样处理这些视觉 token。这需要引入**视觉编码器**（Vision Encoder）和**投影层**（Vision Projector）两个新组件，分别将输入图像编码为视觉特征向量序列和将视觉编码器的输出映射到语言模型的嵌入空间。VLM 的整体架构如下图所示：
...
layers:
  ...
  - {id: embedding, name: "词嵌入层", type: fc, size: "vocab=6400\ndim=768"}
...
SigLIP（Sigmoid Loss for Language-Image Pre-training）是[多模态大模型](./multimodal-llm.md)一章中介绍的 CLIP 家族成员。与原始 CLIP 使用 Softmax 对比损失不同，SigLIP 使用 Sigmoid 损失对图像-文本对进行训练，在 ImageNet 分类和跨模态检索任务上表现优于同等规模的 CLIP 模型。本实验使用 SigLIP2-Base-P32/256 作为视觉编码器，其配置如下：
...
输入 256×256 的图像，块大小 32×32，编码器输出 $8 \times 8 = 64$ 个块 token，每个 token 是 768 维向量，与语言模型的隐藏维度恰好一致，这并不是巧合，设计投影层的目的就是为了让两者在同一个向量空间中对齐。
...
视觉编码器的输出是 64 个 768 维的块 token，它们处于 SigLIP 的特征空间。语言模型的词嵌入也产生 768 维向量，但处于语言模型的语义空间。两个空间虽然维度相同，但向量的分布和含义完全不同。投影层的作用就是充当两个空间之间的翻译器。投影层采用两层 MLP 结构（LayerNorm → Linear → GELU → Linear）。LayerNorm 先将视觉特征归一化，两次线性变换逐步将视觉特征从 SigLIP 的特征空间映射到语言模型的语义空间，GELU 激活函数在中间引入非线性，使投影层能够学习两个空间之间复杂的映射关系。
...
投影后的视觉 token 需要被插入到文本 token 序列中，才能被 Transformer 层处理。token 注入机制是通过词表中预留一个特殊标记 `<|image_pad|>`，数据集构建时将图像对应的 `<image>` 标记替换为 64 个连续的 `<|image_pad|>` token。模型在词嵌入之后、送入 Transformer 层之前，找到序列中的 `<|image_pad|>` 位置，将这些位置上的词嵌入替换为投影后的视觉特征。
...
相比纯语言模型，视觉语言模型的训练样本有除了有文本，还包含一张或多张图像。本实验使用 Parquet 格式存储数据，每条记录包含 `conversations`（对话内容）和 `image_bytes`（图像原始字节）两个字段。训练分为两个阶段，对应两种不同的数据格式：
...
数据集的标签生成策略值得注意。在纯语言模型的预训练中，每个 token 的下一个 token 都是预测目标，标签与输入只差一个位置。但在视觉指令微调中，我们只想让模型学习生成 AI 的回复，而不是学习预测用户的提问。`generate_labels` 方法通过定位 `<|im_start|>assistant\n` 到 `<|im_end|>\n` 之间的 token，只将这些位置的标签设为对应的 token ID，其余位置标记为 -100（在介绍因果掩码时提到过，这是交叉熵损失自动忽略的标记值）。
...
视觉预训练的目标是让投影层学会将视觉特征翻译为语言模型能理解的向量。这个阶段训练的是模型读图的能力，给定一张图像，模型需要学会用语言描述图像内容。在[多模态模型训练](multimodal-llm.md#训练多模态模型)中提到过，视觉编码器（SigLIP）是冻结参数的，因为视觉编码器已经在数十亿图像-文本对上完成了预训练，具备强大的视觉特征提取能力，无需重新训练。语言模型也已通过文本预训练学会了语言的基本规律。视觉预训练阶段的核心任务是训练投影层，让这个翻译器学会在两个已固化的空间之间建立映射。本阶段训练中，语言模型的参数也是冻结的，只训练投影层。这样做一是大幅减少可训练参数量（从 64M 降到约 1.2M），训练速度快且显存占用低；二是避免训练初期投影层输出不稳定时对语言模型已学到的知识造成破坏。预训练阶段结束后，投影层已经能够产生合理的视觉 token，后续 SFT 阶段再解冻语言模型的首尾层，让模型在稳定的基础上精细调整。
...
视觉预训练阶段约 25 万条样本（原数据集的 20%），序列长度 450，批大小 16。由于只训练投影层（约 1.2M 参数），每步训练的显存占用远低于纯语言模型预训练。1 个 epoch 的耗时因 GPU 而异：

- RTX 3090：约 15-20 分钟
- RTX 4060（8 GB）：约 1-2 小时
- 更低端 GPU 请酌情调整 `batch_size` 和 `accumulation_steps`

峰值显存占用约 3-5 GB，8 GB 显存的 GPU 可以运行。如果显存不足，可减小 `batch_size` 并相应增大 `accumulation_steps`，保持等效批大小不变。

:::

## 第四阶段：视觉指令微调

视觉预训练让模型学会了"看图说话"的基本能力，但预训练数据的对话格式单一（主要是图像描述），模型还无法应对多样的视觉问答场景。视觉指令微调（SFT）阶段使用包含多种任务类型的多轮对话数据，让模型学会根据图像回答各种问题。SFT 阶段与预训练阶段有两个重要区别：

1. **冻结策略不同**：SFT 阶段采用分层策略，除了训练投影层外，还解冻语言模型的首层和末层。首层负责理解注入的视觉 token 语义，末层负责生成更准确的回复。这是考虑到模型的语言主干仅 64M 参数，若全参解冻，LLM 原有的通用语言能力极易被图文任务的梯度稀释，因此选择中间层保持冻结，避免破坏预训练学到的语言知识。
2. **学习率不同**：预训练阶段学习率较高（4e-4），因为投影层从零开始训练需要较大的梯度来快速收敛；SFT 阶段学习率很低（5e-6），因为模型已经具备基本能力，只需要小幅调整参数来适应新的对话格式。
...
SFT 阶段约 58 万条样本（原数据集的 20%），序列长度 768，批大小 4。1 个 epoch 的耗时 RTX 5080 约 1 小时。

峰值显存占用约 2-3 GB（BF16 混合精度），8 GB 以上显存的 GPU 可稳定训练。如果显存不足，可减小 `batch_size` 并相应增大 `accumulation_steps`。SFT 阶段使用较小的学习率（5e-6），训练过程中 loss 的下降幅度不如预训练明显，但模型在对话质量上的提升是显著的。
...
# 视觉编码
pixel_values = {k: v.to(device) for k, v in MiniMindVLM.image2tensor(test_image, model.processor).items()}
...
实验揭示了构建视觉语言模型的几个关键设计决策：

1. **模块化架构**：VLM 的架构可以拆分为视觉编码器、投影层、语言模型三个独立模块，每个模块都有明确的职责边界。视觉编码器负责特征提取，投影层负责空间对齐，语言模型负责理解与生成。这种模块化设计使得我们可以独立选择和替换每个组件，比如将 SigLIP 替换为其他视觉编码器，或将语言模型替换为更大的模型。

2. **分阶段训练**：视觉预训练先让投影层学会将视觉特征翻译到语言空间，SFT 再在稳定的基础上微调整体能力。这种"先对齐再微调"的思路避免了训练初期投影层输出不稳定时对语言模型造成干扰，是小规模 VLM 训练中经过验证的有效策略。
...
4. **保存时排除视觉编码器**：模型保存时排除了视觉编码器的权重，因为它是预训练的公开模型，用户加载时重新下载即可。这种做法将模型文件从约 120M 减小到约 64M，节省存储和传输开销。

64M 语言模型 + 86M 视觉编码器的组合在 VLM 领域属于极小规模。作为参考，LLaVA-1.5 使用 13B 的 LLaMA 和 336M 的 CLIP，GPT-4V 的视觉编码器规模更大。参数量的差距决定了模型能力的上限——本实验的模型能够完成简单的图像描述和基础问答，但无法进行复杂的视觉推理、OCR 识别或细粒度的图像分析。但与纯语言模型的预训练实验一样，本次实验关注的是架构完整性和流程可复现性，在单卡 GPU 上体验从"只能处理文本"到"能看图说话"的跨越，这是千亿参数模型无法提供的学习体验。
