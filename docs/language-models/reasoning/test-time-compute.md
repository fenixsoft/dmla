...
- **蒙特卡洛树搜索**是一种更复杂的搜索策略，结合了探索与利用。它最初在围棋 AI AlphaGo 中大放异彩，2023 年姚顺雨在论文《[Tree of Thoughts](https://arxiv.org/abs/2305.10601)》中将其思想引入 LLM 推理，提出了树搜索 + 语言模型的推理框架。MCTS 利用置信上界（Upper Confidence Bound，UCB）平衡了"选历史得分高的节点"与"选访问次数少的节点"的两种策略的风险，两项之和确保搜索不会陷入局部最优。UCB 公式为：
