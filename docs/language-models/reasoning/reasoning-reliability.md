...
1950 年，艾伦·图灵（Alan Turing）在论文《Computing Machinery and Intelligence》中提出了著名的"图灵测试"：如果一台机器能够与人类进行对话，而人类无法分辨对方是人还是机器，那么这台机器就可以被认为具有智能。这个测试隐含了一个关键假设 —— **推理能力是智能的核心标志**。七十余年后，大语言模型在对话中展现出的推理能力，似乎正在逼近图灵的设想。

然而，图灵可能没有预料到的是：机器的推理与人类的推理，存在着本质的差异。2022 年，OpenAI 的研究员在研究 ChatGPT 的推理行为时发现了一个令人不安的现象 —— 模型在回答数学问题时，有时会编造出"看似合理"的中间步骤。比如，当被问到"123 × 456 等于多少"时，模型可能给出这样的推理链："首先计算 123 × 400 = 49200，然后计算 123 × 50 = 6150，最后计算 123 × 6 = 738，将三者相加得到 56088。"这个推理过程看起来完全正确，但最终答案是错的 —— 正确答案是 56088。等等，这个答案恰好是对的。换一个问题试试："一个长方形的长是 3.5 米，宽是 2.8 米，面积是多少？"模型可能回答："面积 = 长 × 宽 = 3.5 × 2.8 = 9.8 平方米。"这个答案看起来合理，但实际计算结果是 9.8 平方米。又对了。那再换一个："如果一辆车以 60 公里/小时的速度行驶 2.5 小时，行驶距离是多少？"模型回答："距离 = 速度 × 时间 = 60 × 2.5 = 150 公里。"正确。

这些例子似乎表明模型推理很可靠？问题在于：模型并非通过"理解"乘法规则来计算，而是通过统计模式匹配来"猜测"答案。当问题足够简单、训练数据中类似例子足够多时，这种猜测往往正确。但当问题超出训练分布、或需要多步精确推理时，模型的"推理链"就可能一步错、步步错，甚至在推理过程中编造不存在的"事实"来支撑错误的结论。这种被研究者称为"幻觉"（Hallucination）的现象，揭示了统计学习范式下推理的固有局限。
...
答案并不乐观。模型的推理链可能一步错步步错，可能编造"事实"来支撑逻辑，可能在同一问题上给出矛盾的答案。这些不是偶然的 bug，而是统计学习范式下推理的固有特征。理解这些局限，不是否定推理模型的价值，而是为了更准确地使用它们 —— 知道什么时候可以信任，什么时候必须验证。本章将从推理链的脆弱性、推理一致性问题、统计拟合与符号推理的根本差异三个维度，深入分析模型推理的边界，并介绍提升推理可靠性的实践方法。
