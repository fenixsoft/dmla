...
[MoE 模型](../architecture-basics/architecture-evolution.md#稀疏架构)（Mixture of Experts）也可算是一种典型的稀疏化。MoE 模型虽然总参数量巨大，但每次推理只激活少数专家（例如 Mixtral 8×7B 只激活 2 个专家），实际计算量远小于稠密模型。MoE 的推理效率优化主要在于如何减少专家切换时的显存访问开销，以及如何提高单次推理中多个 token 激活同一专家的批处理效率。这已经超出了传统剪枝的范畴，更接近于一种结构化的条件计算，但两者的目标是一致的，都是用更少的计算得到接近稠密模型的输出质量。
