...
推理效率优化的本质，是在答得好与答得快之间找到工程上可落地的平衡点。这个平衡之所以难找，根源在于 Transformer 自回归推理的结构性矛盾，Prefill 是计算密集型的突发任务，Decode 是访存密集型的持续任务，把它们放在同一块 GPU 上执行，容易造成资源争用。这个问题无法通过参数调优解决，原因在于架构层面的资源错配。
