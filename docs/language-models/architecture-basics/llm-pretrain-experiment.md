...
在开始实验之前，请确保已[挂载数据目录](../../appendixes/sandbox.md#数据管理)并下载好 MiniMind 预训练语料，你可以通过 `DMLA-CLI` 工具自动完成该工作：
...
模型使用词表规模为 6400 的 BPE 分词器，词表虽小，但通过 BPE 的子词合并机制，6400 个词表条目就足以覆盖中英文的常见字词组合，未登录词会被拆解为更小的子词单元而非直接标记为 `<unk>`。词表中还包含了 `<|im_start|>` 和 `<|im_end|>` 等对话控制标记，它们是后续 SFT 阶段实现多轮对话的基础，预训练阶段虽然不使用这些标记，但它们已经占据词表中的固定位置，确保预训练权重与 SFT 权重的词表对齐。
...
预训练模型的 loss 从初始的约 8.76（均匀分布的交叉熵 $\ln(6400)$）下降到 1.85 左右，说明模型已经学会了中文和英文文本的基本统计规律。但预训练模型的能力存在明确的边界。最明显的局限是预训练模型只会续写，不会对话。它的训练目标是根据前文预测下一个词，输入"你好"，模型会很自然地把这两个字当成一段文本的开头，继续往下写"你好，欢迎来到……"，而不会意识到这是一个对话的开场、应该回应一句问候。这种对话格式的理解能力并不是预训练能赋予的，需要 SFT 阶段通过大量 `<|im_start|>user\n...<|im_end|>\n<|im_start|>assistant\n` 格式的示范来专门教会模型。

另一个根本性的局限在于知识的存储方式。预训练模型的知识是溶解在参数权重中的，不像数据库那样可以精确检索。模型可能在相关提示下正确地续写出某个事实，但它无法告诉你这个事实的出处，也无法保证不会在稍有不同的提示下编造出另一个版本，这正是 LLM 幻觉问题的根源所在。除此之外，预训练阶段只培育了最基本的语言建模能力，像工具调用、思维链推理、指令遵循这些高级技能，都需要在后续的对齐训练阶段，如监督微调、RLHF 中一步步嫁接上去。
